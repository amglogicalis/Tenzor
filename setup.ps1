# setup.ps1
# Script de Instalación y Onboarding para Arzor AIs CLI (Windows PowerShell)
...
if (-not $pythonPath) {
    ...
    Write-Host "    Descárgalo de https://www.python.org/ e inténtalo de nuevo." -ForegroundColor Yellow
    Exit 1
}
...
Write-Host ""
Write-Host "        ¿A qué dirección URL de servidor de Arzor deseas conectarte?" -ForegroundColor Yellow
Write-Host "        [Enter para usar producción: $defaultUrl]" -ForegroundColor DarkGray
...
# 5. Configurar el PATH de Windows de forma automática
Write-Host ""
...
if (-not (Test-Path $scriptsPath)) {
    # Fallback si no existe la carpeta específica de roaming de la versión, buscar la de sistema
...
if ($userPath -notlike "*$scriptsPath*") {
    Write-Host "        Añadiendo $scriptsPath a tu PATH de usuario de Windows..." -ForegroundColor Gray
    [Environment]::SetEnvironmentVariable("PATH", $userPath + ";$scriptsPath", "User")
    Write-Host "        ✔ PATH de usuario actualizado con éxito." -ForegroundColor Green
    $pathUpdated = $true
} else {
    Write-Host "        La carpeta de scripts ya está en tu PATH de Windows." -ForegroundColor Gray
    $pathUpdated = $false
}

# 6. Autenticación Inicial
Write-Host ""
Write-Host "  🎉 ¡Onboarding del CLI completado con éxito! 🎉" -ForegroundColor Green
Write-Host ""

if ($pathUpdated) {
    Write-Host "  ⚠️  IMPORTANTE: Cierra esta consola de PowerShell y abre una ventana nueva" -ForegroundColor Yellow
